...
class RockGenerator:
    ...
    def update_instancer(self, region: BoundingBox, rocks_data): # -> None:
        """
        ...
        position, orientation, scale, ids = self.aggregate_block_data(blocks) # C'est ca ki kompte

        self.rock_instancer.set_instancer_parameters(position, orientation, scale, ids)
        rocks_data += [position, scale]    

    def sample(self, global_position: Tuple[float, float], rocks_data): # -> None:
        """
        ...
        self.update_instancer(region, rocks_data)
        self.is_sampling = False
        ...
        if self.check_if_update_needed(position):
            ...
            for rock_generator in self.rock_generators:
                with ScopedTimer("RockManager sample", active=self.settings.profiling):
                    rock_generator.sample(position, rocks_data)
