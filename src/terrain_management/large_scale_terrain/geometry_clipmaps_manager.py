...
class GeoClipmapManager:
    ...
    def update_geoclipmap(self, position: np.ndarray, mesh_position: np.ndarray) -> None:
        ...
        # Nasty trick to force renderer update
        val = self._stage.GetPrimAtPath(self._mesh_path).GetAttribute("primvars:doNotCastShadows").Get()
        self._stage.GetPrimAtPath(self._mesh_path).GetAttribute("primvars:doNotCastShadows").Set(not val)
    ...
    def render_mesh(
        ...
        if update_topology:
            ...
            cs = UsdGeom.PrimvarsAPI(mesh.GetPrim()).CreatePrimvar("doNotCastShadows", Sdf.ValueTypeNames.Bool)
            cs.Set(True)
