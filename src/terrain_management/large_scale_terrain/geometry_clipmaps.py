...
class GeoClipmap:
    ...
    def get_height_and_random_orientation(
        ...
        if self.acceleration_mode == "hybrid":
            ...
            return self.DEM_sampler.bilinear_interpolation_and_normal(x, y, coords, seed=seed)
...
class DEMSampler:
    ...
    def bilinear_interpolation_and_normal(
        self,
        x: np.ndarray,
        y: np.ndarray,
        coords: Tuple[float, float],
        ...
        with wp.ScopedTimer("bilinear_interpolation_and_normal", active=self.profiling):
            with wp.ScopedTimer("creating buffers", active=self.profiling):
                # Create buffers
                x_wp = wp.array(x, dtype=float, device="cuda", copy=True)
                y_wp = wp.array(y, dtype=float, device="cuda", copy=True)
                z = wp.zeros(x.shape[0], dtype=float, device="cuda")
                q = wp.zeros(x.shape[0], dtype=wp.mat22f, device="cuda")
                normals = wp.zeros(x.shape[0], dtype=wp.vec3f, device="cuda")
                quat = wp.zeros(x.shape[0], dtype=wp.quatf, device="cuda")
                map_offset_wp = wp.vec2f(self.dem_center[0], self.dem_center[1])
            with wp.ScopedTimer("preprocessing", active=self.profiling):
                wp.launch(
                    kernel=_preprocess_multi_points,
