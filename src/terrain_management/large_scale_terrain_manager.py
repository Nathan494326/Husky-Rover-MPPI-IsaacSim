...
class LargeScaleTerrainManager:
    ...
    def get_local_position(self, coordinates: Tuple[float, float]) -> float:
        """
        Get the pixel coordinates at the given coordinates in the local map.
        ...
        return self.map_manager.get_coordinates_local(global_coordinates)
    ...
    def update_visual_mesh(self, local_coordinates):
        ...
        if dist > self.settings.update_every_n_meters:
            ...
            updated = self.map_manager.hr_dem_gen.update_terrain_data_blocking(global_corrected_coordinates)
            if updated:
                self.nested_clipmap_manager.update_DEM()
            ...
            self.rocks_data = []
            self.rock_manager.sample(local_coordinates, self.rocks_data)
