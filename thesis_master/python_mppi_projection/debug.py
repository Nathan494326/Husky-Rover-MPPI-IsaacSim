...
def plot_trajectories_2D(trajectories):
    ...
    for idx, traj in enumerate(trajectories):
        ...
        # Plot the trajectory
        plt.plot(x, y, label=f"Trajectory {idx+1}")
...
def find_corners_heights(x, y, q, resolution, X, Y, Z):
    ...
    i = np.max(np.searchsorted(X[0], x0), 0)  # x index
    j = np.max(np.searchsorted(Y[:, 0], y0), 0)  # y index
...
x0, y0 = 6, -6
heading_vector = np.array([-1.0, 4.0, 0])
...
h = bilinear_interpolator(1, -18, q, resolution)
normal = normal_on_grid(q, resolution)
hv = get_heading_tangent_vector(normal, heading_vector)

a, b, c = update_position(1, -18, hv, -1, -0.2, normal, dt)
