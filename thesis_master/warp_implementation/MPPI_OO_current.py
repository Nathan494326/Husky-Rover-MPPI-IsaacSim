...
def plot_2d_surface_with_trajectory(X, Y, Z, costmap, trajectories, show_live, frame, frame_folder='frame_folder'):
    """
    ...
    img = ax.imshow(Z, cmap="jet", origin="lower", extent=[X.min(), X.max(), Y.min(), Y.max()])
    plt.colorbar(img, label="Height")

    # Overlay costmap points (thresholded)
    above_threshold = costmap > 0.99
    ax.scatter(X[above_threshold], Y[above_threshold], color='red', s=1, label="High Cost Areas")
...
class Surface:
    def __init__(self, which_map, filename, which_costmap, costmap_file, grid_size, half_width, bumps, radius_robot, obstacles=[]):
        ...
        elif which_map == "imported":
            start_index = 1000
            end_index = 2500
            self.resolution = 2 * self.half_width / (end_index-start_index)
...
class MPPI_Controller:
    ...
    def run(self, proj):
        ...
        while (abs(self.robot.x[-1] - self.goal_x) > 0.5 or abs(self.robot.y[-1] - self.goal_y) > 0.5) and self.loop < 3500:
            ...
            self.std_dev_u1 = np.maximum(0.3, 0.3 - ang_vel*ang_vel)
            self.std_dev_u2 = np.maximum(0.3, 0.3 + ang_vel*ang_vel)
